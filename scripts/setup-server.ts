...
async function makeCategory(guild: Guild, name: string): Promise<CategoryChannel> {
  return guild.channels.create({ name, type: ChannelType.GuildCategory }) as Promise<CategoryChannel>;
}
...
async function setupOfficial(guild: Guild) {
  ...
  const keeper = await guild.roles.create({ name: 'keeper', color: C.ember,      hoist: true,  permissions: [PermissionFlagsBits.Administrator] });
  const tender = await guild.roles.create({ name: 'tender', color: C.deepEmber,  hoist: true,  permissions: [PermissionFlagsBits.ManageMessages, PermissionFlagsBits.KickMembers, PermissionFlagsBits.ModerateMembers] });
  ...
  await           guild.roles.create({ name: 'hearth', color: C.charcoal,   hoist: false, permissions: [PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks] });
  ...
  const welcome = await makeChannel(guild, 'welcome', catStart,
    readonlyOverwrites(everyone.id, keeper.id));
  const rules = await makeChannel(guild, 'rules', catStart,
    readonlyOverwrites(everyone.id, keeper.id));

  const catUpdates = await makeCategory(guild, '🔔 updates');
  const announcements = await makeChannel(guild, 'announcements', catUpdates,
    readonlyOverwrites(everyone.id, keeper.id));
  await makeChannel(guild, 'changelog', catUpdates,
    readonlyOverwrites(everyone.id, keeper.id));
...
async function setupDev(guild: Guild) {
  ...
  const architect  = await guild.roles.create({ name: 'architect',    color: C.ember,      hoist: true,  permissions: [PermissionFlagsBits.Administrator] });
  const maintainer = await guild.roles.create({ name: 'maintainer',   color: C.deepEmber,  hoist: true,  permissions: [PermissionFlagsBits.ManageMessages] });
  ...
  await              guild.roles.create({ name: 'hearth [dev]',  color: C.ashGrey,    hoist: false, permissions: [PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks] });
  ...
  const roadmap  = await makeChannel(guild, 'roadmap', catMeta,
  ...
  await makeChannel(guild, 'pull-requests', catDev, readonlyOverwrites(everyone.id, maintainer.id));
  ...
  await               makeChannel(guild, 'logs', catTesting,
